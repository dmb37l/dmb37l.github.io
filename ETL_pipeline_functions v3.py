...
def clean_actdets( actdets, activity_id):
    ...
    effort_types = list(map(lambda x: x['type'], actdets))
    effort_buckets_nested = list(map(lambda x: x['effort_buckets'], actdets))
    effort_buckets = [list(map(lambda y: y['time'], x)) for x in effort_buckets_nested]
    efforts = list(zip(effort_types, effort_buckets))


    
    
            cleaned_zone['zone_type'] =  effort[0]
            cleaned_zone['zone_index'] = i + 1
            cleaned_zone['time'] = effort[1][i]
    cleaned_actdets = []
    
    for effort in efforts:
        for i in range(len(effort[1])):
        cleaned_actdet = {}
